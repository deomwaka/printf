 ...
 * and stores it to buffer contained in struct
		...
		unsigned char flags, int wid, int prec, unsigned char len)
{
	...
	if (prec <= 0 && NEG_FLAG == 0)
	{
		...
		/* Print negative sign when when zero flag is active */
		if (ZERO_FLAG == 1 && d < 0)
 ...
 * stores it to buffer cointained in struct
 * @args: a va_list pointing to argument to be converted
 * @flags: flag modifier
 * @wid: width modifier
 * @prec: precision modifier
 ...
 * stores it to buffer contained in struct
 * @args: a va_list pointing to argument to be converted
 * @flags: flag modifier
 * @wid: width modifier
 * @prec: precision modifier
